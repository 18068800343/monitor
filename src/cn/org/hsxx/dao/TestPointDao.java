...
public class TestPointDao {
	...
	public List<TestPoint> getTestPointData(String bridge_id,String cd_f_type_id){
		...
		String sql = "select tp.id,tp.modelID,tp.cd_code,tp.td_code,tp.cd_span_no,bdj.`id` as bdjid,tp.yj_status,bdj.`name`,tp.fz,tp.cd_describe,tp.cypl,tp.sbxh,tp.if_bad,tp.if_jihuo from test_point tp LEFT JOIN basic_data_jclx bdj ON bdj.id = tp.cd_type_id WHERE tp.bridge_id = ?";
	...
	public int saveTestPoint(TestPoint tp){
		...
		String sql="INSERT INTO test_point(id,cd_code,td_code,cd_span_no,s_id,fz,cd_describe,cypl,sbxh,cd_type_id,cd_f_type_id,bridge_id,r_id,if_jihuo) values(?,?,?,?,?,?,?,?,?,?,?,?,?,?);";
		MyDataOperation dataOperation = new MyDataOperation(MyDataSource.getInstance().getConnection());
	...
	public int updateTestPoint(TestPoint tp){
		String sql = "UPDATE test_point SET cd_code=?,td_code=?,cd_span_no=?,cd_type_id=?,fz=?,cd_describe=?,cypl=?,sbxh=?,yj_status=? WHERE id=?";
		MyDataOperation dataOperation = new MyDataOperation(MyDataSource.getInstance().getConnection());
		int i = dataOperation.executeUpdate(sql, new Object[]{tp.getCd_code(),tp.getTd_code(),tp.getCd_span_no(),tp.getCd_type_id(),tp.getFz(),tp.getCd_describe(),tp.getCypl(),tp.getSbxh(),tp.getYjStatus(),tp.getId()});
		dataOperation.close();
		return i;
	}
